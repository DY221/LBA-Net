...
OUT_DIR  = "/home/wang/ultrasound/deeplabv3p-buet"
...
# ================= DeepLabv3+ (using timm) =================
def create_deeplabv3plus(num_classes=1, pretrained=True):
    model = smp.DeepLabV3Plus(
        encoder_name="resnet50",           # 可改 xception / efficientnet-b3 等
        encoder_weights="imagenet" if pretrained else None,
        in_channels=3,
        classes=num_classes,
        activation=None                    # 我们自己做 sigmoid
    )
    return model
...
def evaluate_fold_model(model_path, val_indices, full_dataset):
    ...
    model = create_deeplabv3plus(num_classes=1, pretrained=True).to(device)
...
if __name__ == "__main__":
    ...
    model = create_deeplabv3plus(num_classes=1, pretrained=True).to(device)
    summary(model, (3, 512, 512))   # 确认参数量、ASPP、Decoder 均出现
