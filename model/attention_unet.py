...
OUT_DIR = "/home/wang/ultrasound/attentionunet-BUET"
...
class AttentionGate(nn.Module):
    def __init__(self, in_channels, gating_channels, inter_channels):
        ...
        # W_g, W_x are the 1x1 convolutions that map the gating and input features to the same dimension.
        ...
        # psi is the final convolution to reduce the attention map to a scalar
        self.psi = nn.Conv2d(inter_channels, 1, kernel_size=1)
        self.relu = nn.ReLU(inplace=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, g):
        # x is the encoder feature map, g is the decoder feature map
        g1 = self.W_g(g)  # [B, C, H, W]
        x1 = self.W_x(x)  # [B, C, H, W]
        
        # Add the gating and input features together, apply ReLU activation
        psi = self.relu(g1 + x1)  # [B, C, H, W]
        
        # Apply the psi convolution to get the attention map
        psi = self.psi(psi)  # [B, 1, H, W]
        
        # Use Sigmoid to normalize the attention map between 0 and 1
        alpha = self.sigmoid(psi)  # [B, 1, H, W]
        
        # Multiply the input feature map by the attention map
        return x * alpha  # [B, C, H, W] output
...
class AttentionUNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        ...
        self.att4 = AttentionGate(512, 512, 256)  # AttentionGate
        self.dec4 = DoubleConv(1024, 512)

        self.up3 = nn.ConvTranspose2d(512, 256, kernel_size=2, stride=2)
        self.att3 = AttentionGate(256, 256, 128)  # AttentionGate
        self.dec3 = DoubleConv(512, 256)

        self.up2 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.att2 = AttentionGate(128, 128, 64)  # AttentionGate
        self.dec2 = DoubleConv(256, 128)

        self.up1 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        self.att1 = AttentionGate(64, 64, 32)  # AttentionGate
        self.dec1 = DoubleConv(128, 64)

        # Final 1x1 convolution to reduce the channels to `num_classes`
    ...
    def forward(self, x):
        ...
        d4 = self.up4(b)
        d4 = self.att4(d4, e4)  # Apply AttentionGate
        d4 = torch.cat([d4, e4], dim=1)  # Concatenate with skip connection
        d4 = self.dec4(d4)

        d3 = self.up3(d4)
        d3 = self.att3(d3, e3)  # Apply AttentionGate
        d3 = torch.cat([d3, e3], dim=1)  # Concatenate with skip connection
        d3 = self.dec3(d3)

        d2 = self.up2(d3)
        d2 = self.att2(d2, e2)  # Apply AttentionGate
        d2 = torch.cat([d2, e2], dim=1)  # Concatenate with skip connection
        d2 = self.dec2(d2)

        d1 = self.up1(d2)
        d1 = self.att1(d1, e1)  # Apply AttentionGate
        d1 = torch.cat([d1, e1], dim=1)  # Concatenate with skip connection
        d1 = self.dec1(d1)

        out = self.final_conv(d1)  # Output layer
        return out  # [B, 1, H, W]
