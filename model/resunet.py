...
OUT_DIR = "/home/wang/ultrasound/resunet-BUET"
...
def hd95_score(pred, target):
    ...
    target_onehot = torch.cat([1 - target_bin, target_bin], dim=1)

    hd95_vals = compute_hausdorff_distance(
        y_pred=pred_onehot,
        y=target_onehot,
        include_background=False,  # 只算前景
        percentile=95,
        directed=False,
        spacing=None
    )   # 返回 [B] 或 [B,1]，视版本而定

    hd95_vals = torch.nan_to_num(hd95_vals, nan=0.0, posinf=0.0, neginf=0.0)
    return hd95_vals.mean().item()


# ================= 标准 ResUNet 结构（适合论文对比使用） =================
class ResidualConvBlock(nn.Module):
    """
    标准残差卷积块:
    Conv(3x3) -> BN -> ReLU -> Conv(3x3) -> BN
    + identity (如果通道或尺寸不一致，用1x1 conv对齐)
    然后 ReLU
    """
    def __init__(self, in_ch, out_ch, stride=1):
        ...
        # 如果输入输出通道或步长不一致，用 1x1 conv 做下采样匹配
...
class ResUNet(nn.Module):
    ...
    标准的 ResUNet:
    - 编码器每层是残差块 + maxpool
    - 解码器：上采样 (ConvTranspose2d) + concat(skip) + 残差块
    通道设置: 64-128-256-512-1024，适合 512x512 输入，用于论文对比比较常见
    """
    def __init__(self, in_channels=3, num_classes=1):
        super().__init__()

        # Encoder
        self.enc1 = ResidualConvBlock(in_channels, 64)   # H, W
        self.enc2 = ResidualConvBlock(64, 128)           # H/2, W/2
        self.enc3 = ResidualConvBlock(128, 256)          # H/4, W/4
        self.enc4 = ResidualConvBlock(256, 512)          # H/8, W/8

        # Bottleneck
        self.bottleneck = ResidualConvBlock(512, 1024)   # H/16, W/16
    ...
    def forward(self, x):
        ...
        p1 = self.pool(e1)             # [B, 64, 256, 256]

        e2 = self.enc2(p1)             # [B, 128, 256, 256]
        p2 = self.pool(e2)             # [B, 128, 128, 128]

        e3 = self.enc3(p2)             # [B, 256, 128, 128]
        p3 = self.pool(e3)             # [B, 256, 64, 64]

        e4 = self.enc4(p3)             # [B, 512, 64, 64]
        p4 = self.pool(e4)             # [B, 512, 32, 32]

        # Bottleneck
        b = self.bottleneck(p4)        # [B, 1024, 32, 32]
