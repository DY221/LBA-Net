...
segmentation_loss = nn.BCEWithLogitsLoss()  # 或使用 dice_loss（自定义）
...
OUT_DIR = "/home/wang/ultrasound/fpn-BUET"
...
def hd95_score(pred, target, img_size=512):
    ...
    try:
        hd95_vals = compute_hausdorff_distance(
            y_pred=pred_oh,
            y=target_oh,
            include_background=False,  # 只计算前景
            percentile=95,
            directed=False,
            spacing=None
        )  # shape: [B, 1]

        # 提取每个样本的HD95值，并处理NaN/Inf
...
class StandardFPN(nn.Module):
    def __init__(self, backbone_name='resnet34', num_classes=1, pretrained=True):
        ...
        # Lateral convolutions (1x1卷积调整通道数)
        self.lateral_conv2 = nn.Conv2d(encoder_channels[0], fpn_channels, 1)
        self.lateral_conv3 = nn.Conv2d(encoder_channels[1], fpn_channels, 1)
        self.lateral_conv4 = nn.Conv2d(encoder_channels[2], fpn_channels, 1)
        self.lateral_conv5 = nn.Conv2d(encoder_channels[3], fpn_channels, 1)
        
        # Smooth convolutions (3x3卷积消除上采样伪影)
        self.smooth_conv2 = nn.Conv2d(fpn_channels, fpn_channels, 3, padding=1)
        self.smooth_conv3 = nn.Conv2d(fpn_channels, fpn_channels, 3, padding=1)
        self.smooth_conv4 = nn.Conv2d(fpn_channels, fpn_channels, 3, padding=1)
        self.smooth_conv5 = nn.Conv2d(fpn_channels, fpn_channels, 3, padding=1)
        
        # 移除特征融合卷积（标准FPN不需要）
        # 每个层级独立的分割头
        self.seg_heads = nn.ModuleList([
            self._make_seg_head(fpn_channels, num_classes) for _ in range(4)
        ])
    ...
    # 自顶向下路径 with 横向连接（标准FPN）
        p5 = self.lateral_conv5(c5)
        p5 = self.smooth_conv5(p5)
    
        p4 = self._upsample_add(p5, self.lateral_conv4(c4))
    ...
    # 每个尺度输出独立分割头
        p2_out = self.seg_heads[0](p2)
        p3_out = self.seg_heads[1](p3)
        p4_out = self.seg_heads[2](p4)
        p5_out = self.seg_heads[3](p5)
    
    # 将所有预测上采样到目标尺寸（512x512）
        target_size = x.size()[2:]  # 获取输入图像的尺寸（H, W），例如 [512, 512]
        p2_final = F.interpolate(p2_out, size=target_size, mode='bilinear', align_corners=True)
        p3_final = F.interpolate(p3_out, size=target_size, mode='bilinear', align_corners=True)
        p4_final = F.interpolate(p4_out, size=target_size, mode='bilinear', align_corners=True)
        p5_final = F.interpolate(p5_out, size=target_size, mode='bilinear', align_corners=True)
    
    # 多尺度预测融合
        final_output = (p2_final + p3_final + p4_final + p5_final) / 4
    
        return final_output


# ================= 4. 改进的损失函数 =================
def multi_scale_segmentation_loss(seg_logits, mask, scale_weights=[0.4, 0.3, 0.2, 0.1]):
    """多尺度分割损失"""
    total_loss = 0
    for i, logit in enumerate(seg_logits):  # seg_logits现在是多尺度输出列表
        # 将mask下采样到对应尺度
        scale_mask = F.interpolate(mask, size=logit.shape[2:], mode='bilinear', align_corners=True)
        scale_loss = segmentation_loss(logit, scale_mask)
...
def visualize_with_guidance(model, val_loader, epoch, save_dir, num_samples=3):
    ...
    if isinstance(epoch, int):
        plt.savefig(f'{save_dir}/unet_visualization_epoch_{epoch:03d}.png', dpi=100, bbox_inches='tight')
    else:
        plt.savefig(f'{save_dir}/unet_visualization_{epoch}.png', dpi=100, bbox_inches='tight')
    plt.close()
...
def evaluate_fold_model(model_path, val_indices, full_dataset):
    ...
    model = StandardFPN(backbone_name='resnet34', pretrained=True).to(device)  # 修改为 StandardFPN
...
def train_kfold_boundary_guided(k_folds=5):
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(full_ds.imgs)):
        ...
        # 初始化模型、优化器、调度器
        model = StandardFPN(backbone_name='resnet34', pretrained=True).to(device)
        ...
        plt.subplot(1, 2, 2);
        plt.plot(val_dices);
        plt.title(f'Fold {fold + 1} Val Dice')
...
def compute_flops_params(model, input_size=(1, 3, IMG_SIZE, IMG_SIZE)):
    model.eval()
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    print(f"训练轮数: {EPOCHS}")
    # 在训练前打印模型复杂度
    dummy_model = StandardFPN(backbone_name='resnet34', pretrained=False).to(device)
    flops, params = compute_flops_params(dummy_model)
